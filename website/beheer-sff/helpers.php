...
function secure_session_start(): void {
    if (session_status() === PHP_SESSION_NONE) {
        ...
        // Bepaal cookie pad project-specifiek: als we in beheer-*/ zitten,
        // gebruik het parent directory zodat mail.php en beheer/ dezelfde sessie delen.
        $scriptDir = dirname($_SERVER['SCRIPT_NAME'] ?? '/');
        if (strpos(basename($scriptDir), 'beheer') === 0) {
            $cookiePath = dirname($scriptDir) . '/';
        } else {
